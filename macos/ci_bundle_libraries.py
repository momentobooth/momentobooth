...
def copy_and_relink_dependency(dep, executable_path, search_folders, target_file):
    ...
    for folder in search_folders:
        ...
        if os.path.exists(candidate_path):
            ...
            print(
                f"Changing dependency in {target_file}: {dep} -> @executable_path/../Libs/{os.path.basename(output_path)}"
            )
            subprocess.run(
                [
                    "install_name_tool",
                    "-change",
                    dep,
                    f"@executable_path/../Libs/{os.path.basename(output_path)}",
                    target_file,
                ]
            )
            return output_path
...
def process_directory(directory_path, output_folder, search_folders, processed_files):
    for root, _, files in os.walk(directory_path):
        for file_name in files:
            file_path = os.path.join(root, file_name)
            process_file(file_path, output_folder, search_folders, processed_files)
...
def main(input_path, output_folder, search_folders):
    ...
    if os.path.isfile(input_path):
        ...
        process_directory(input_path, output_folder, search_folders, processed_files)
    else:
